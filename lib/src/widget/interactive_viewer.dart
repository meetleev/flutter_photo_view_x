...
/// See also:
///
///   * [InteractiveViewerX.builder], whose builder is of this type.
...
///     which includes the use of InteractiveViewerX.
...
/// {@end-tool}
@immutable
class InteractiveViewerX extends StatefulWidget {
  /// Create an InteractiveViewerX.
  ///
  ...
  ///
  /// Passed with the [InteractiveViewerX.builder] constructor. Otherwise, the
  ...
  /// The child [Widget] that is transformed by InteractiveViewerX.
  ///
  /// If the [InteractiveViewerX.builder] constructor is used, then this will be
  ...
  /// is often useful when a child should be bigger than the InteractiveViewerX.
  ///
  ...
  ///
  /// {@template flutter.widgets.InteractiveViewerX.onInteractionEnd}
  ...
  ///
  /// {@macro flutter.widgets.InteractiveViewerX.onInteractionEnd}
  ///
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      InteractiveViewerX.getNearestPointOnLine(point, quad.point0, quad.point1),
      InteractiveViewerX.getNearestPointOnLine(point, quad.point1, quad.point2),
      InteractiveViewerX.getNearestPointOnLine(point, quad.point2, quad.point3),
      InteractiveViewerX.getNearestPointOnLine(point, quad.point3, quad.point0),
    ];
  ...
  @override
  State<InteractiveViewerX> createState() => InteractiveViewerXState();
}

class InteractiveViewerXState extends State<InteractiveViewerX>
    with TickerProviderStateMixin, BoundaryHitDetector {
  TransformationController get _transformationController => widget.transformationController;
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    switch (_gestureType!) {
      case _GestureType.scale:
        ...
        _onScaleChange(_transformationController.value.getMaxScaleOnAxis());
  ...
  @override
  void didUpdateWidget(InteractiveViewerX oldWidget) {
    ...
    if (widget.transformationController != oldWidget.transformationController) {
      oldWidget.transformationController.removeListener(_onTransformationControllerChange);
      widget.transformationController.addListener(_onTransformationControllerChange);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.child != null) {
      ...
      // When using InteractiveViewerX.builder, then constrained is false and the
    ...
    return Listener(
      ...
      child: PhotoGestureDetector(
        ...
        onDoubleTapDown: (TapDownDetails details) => _doubleTapLocalPosition = details.localPosition,
  ...
  void _onDoubleTap() {
    ...
    double offSetX = targetScale == 1.0 ? 0 : -_doubleTapLocalPosition!.dx * (targetScale - 1);
    double offSetY = targetScale == 1.0 ? 0 : -_doubleTapLocalPosition!.dy * (targetScale - 1);

    var destMatrix = Matrix4.fromList([
      targetScale,
      matrix.row1.x,
      matrix.row2.x,
      matrix.row3.x,
    ...
    _scaleAnimation = Tween<Matrix4>(
      begin: matrix,
      end: destMatrix,
    ).animate(
      CurveTween(curve: Curves.easeOut).animate(_controller),
    );

    _scaleAnimation?.addListener(_onDoubleAnimation);
  ...
  void _onDoubleAnimation() {
    if (_controller.isCompleted) {
      ...
      if (currentScale != _targetScale) {
        _transformationController.value.setEntry(0, 0, _targetScale!);
        _transformationController.value.setEntry(1, 1, _targetScale!);
        _transformationController.value.setEntry(2, 2, _targetScale!);
      }
      _onScaleChange(_targetScale!);
      return;
    }
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return InteractiveViewerX.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside = InteractiveViewerX.getNearestPointInside(point, boundary);
